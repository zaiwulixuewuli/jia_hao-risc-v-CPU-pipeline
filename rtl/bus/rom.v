module rom(
    ...
    initial begin
        ...
        */
        ...
        // 人工插入气泡，规避所有数据冒险
        /*
        rom_mem[0] = 32'h00a00013;  // addi x0, x0, 10
        rom_mem[1] = 32'h10000237;  // lui  x4, 10000    (产生 x4)
        rom_mem[2] = 32'h01400193;  // addi x3, x0, 20   (产生 x3)
        rom_mem[3] = 32'h00000013;  // NOP
        rom_mem[4] = 32'h00000013;  // NOP
        rom_mem[5] = 32'h00000013;  // NOP (等 x3, x4 写回寄存器)
        rom_mem[6] = 32'h00322823;  // sw   x3, 16(x4)   (安全使用 x3, x4)
        rom_mem[7] = 32'h00000013;  // NOP
        rom_mem[8] = 32'h00000013;  // NOP
        rom_mem[9] = 32'h01022283;  // lw   x5, 16(x4)   (产生 x5)
        rom_mem[10]= 32'h00000013;  // NOP
        rom_mem[11]= 32'h00000013;  // NOP
        rom_mem[12]= 32'h00000013;  // NOP (等 x5 读出并写回寄存器)
        rom_mem[13]= 32'h00528313;  // addi x6, x5, 5    (安全使用 x5)
        rom_mem[14]= 32'h003303b3;  // add  x7, x6, x3
        // ... 后面的跳转测试先不管，先看 x5 和 x6 对不对
        */
